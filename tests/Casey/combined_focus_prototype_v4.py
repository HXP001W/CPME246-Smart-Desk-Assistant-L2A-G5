...
while True:
    ...
    active_window_info = {"window_title": "UNAVAILABLE", "source": "none", "available": False}
    app_focus_state_current = "UNKNOWN_APP"
    ...
    cv2.putText(frame, f"App(current): {app_focus_state_current}", (20, 150),
                FONT, 0.6, (255, 220, 120), 2)

    cv2.putText(frame, f"App(smoothed): {app_focus_state}", (20, 180),
                FONT, 0.6, (255, 220, 120), 2)
    ...
    cv2.putText(frame, f"State: {combined_state}", (20, 220),
    ...
    cv2.putText(frame, f"Escalation: {escalation_state}", (20, 260),
                FONT, 0.8, escalation_color, 2)

    cv2.putText(frame, f"Distraction timer: {distraction_duration:.1f}s", (20, 295),
                FONT, 0.6, (0, 150, 255), 2)

    cv2.putText(frame, f"Posture timer: {posture_duration:.1f}s", (20, 325),
                FONT, 0.6, (255, 150, 0), 2)

    # Active window title (truncated for display)
    window_title_display = active_window_info.get("window_title", "UNAVAILABLE")
    if len(window_title_display) > 60:
        window_title_display = window_title_display[:57] + "..."

    cv2.putText(frame, f"Active window: {window_title_display}", (20, frame_h - 80),
                FONT, 0.55, (220, 220, 220), 2)
