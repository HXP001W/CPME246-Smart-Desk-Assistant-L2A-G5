# 4-LED Environmental Light Control System with Web UI
...
# LED Pins (GPIO numbers, not physical pin numbers)
LED_PINS = [22, 23, 24, 25]
leds = [LED(pin) for pin in LED_PINS]

# Light Sensor Pin (GPIO27, Physical Pin 13)
LIGHT_SENSOR_PIN = 27
...
manual_led_level = 0  # 0-4, number of LEDs to turn on in manual mode
system_running = True

# ==================== LED CONTROL FUNCTION ====================
def set_led_level(level):
    """
    Set LED brightness level (0-4)
    0 = all off, 4 = all on
    """
...
def auto_mode_thread():
    ...
    while system_running:
        if system_mode == "auto":
            try:
                # Read light sensor: 0 = full dark, 1 = full bright
                light_level = light_sensor.value
                # Invert: darker = more LEDs on, brighter = fewer LEDs on
                # Map 0-1 light level to 0-4 LED level
        ...
        function updateStatus() {
            fetch('/get_status')
                ...
                .then(data => {
                    ...
                    // Update slider value in manual mode
                    if (data.mode === 'manual') {
...
if __name__ == '__main__':
    try:
        ...
        set_led_level(0)
        print("System stopped safely, all LEDs are OFF")
